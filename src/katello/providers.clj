...
(nav/defpages :katello.deployment/any katello.menu
  [::custom-page
   ...
   [::named-page (fn [ent] (nav/choose-left-pane (kt/provider ent)))
    [::products-page (nav/browser-fn (browser/click ::products-and-repositories)
                                     #_(sleep 2000))
...
(defn- add-product
  ...
  (when gpg-key (browser/select-by-text ::prd-gpg-select gpg-key))
  (browser/quick-fill [::product-name-text (or name "")
...
(defn- update-product
  ...
  (when (not= (:gpg-key product) gpg-key)
    (nav/go-to product)
    (browser/click  ::update-prd-gpg-keys)
    (browser/select ::prd-gpg-select gpg-key)
